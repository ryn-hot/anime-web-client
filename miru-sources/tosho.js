...
export default new class Tosho extends AbstractSource {
  ...
  async all ({ anidbAid }) {
    ...
    // Here we skip buildQuery() entirely to do an unfiltered fetch.
    // If you *do* want to support optional exclusions/resolution, 
    // just adapt the buildQuery usage accordingly:
    //    const query = this.buildQuery({ resolution: "", exclusions: [] })
    ...
    // Wait for sneedex to finish
    await sneedexPromise

    // Convert raw data to your standard result format
