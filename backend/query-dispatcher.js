...
async function nyaa_function_dispatch(nyaa_queries, release_status_complete, fallback) {
    ...
    if (unique_torrents.length >= 1) {
        ...
        if (release_status_complete && !fallback) {
            ...
            const checked_reserve_torrents = await nyaa_reserve_extract(uniq_reserve_torrents, nyaa_queries[0][2], nyaa_queries[1][2],  episode, nyaa_queries[0][5], nyaa_queries[0][6]);
